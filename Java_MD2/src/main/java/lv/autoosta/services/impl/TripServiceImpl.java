...
public class TripServiceImpl implements ITripService {
	...
	public ArrayList<Trip> selectAllTripsByDriverId(long idd) throws Exception {
		if (idd > 0) {
			if (driverRepo.existsById(idd)) {
				...
				throw new Exception("Driver with the specified id not found!");
			}
		} else {
	...
	public Trip changeTripDriverByDriverId(long idd, long idtr) throws Exception {
		if (idd > 0 && idtr > 0) {
			if (driverRepo.existsById(idd) && tripRepo.existsById(idtr)) {
				Trip tempTrip = tripRepo.findById(idtr).get();
				tempTrip.setDriver(driverRepo.findByIdd(idd));
				tripRepo.save(tempTrip);
				return tempTrip;
			} else {
				throw new Exception("Driver or trip with specified id does not exist!");
			}
		} else {
			throw new Exception("Incorrect id!");
		}
	...
	@Override
	public Trip selectTripByIdtr(long idtr) {
