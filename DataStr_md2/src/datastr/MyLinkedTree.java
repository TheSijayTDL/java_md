...
public class MyLinkedTree<T> {
	...
	public MyLinkedTree(String expression) {
		this.root = expressionTree(expression);
	}
	...
	private MyNode<T> expressionTree(String expression) {
		MyStack<MyNode<T>> nodeStack = new MyStack<>();
		...
		try {
			for (int i = 0; i < expression.length(); i++) {
				...
				if (Character.isDigit(current)) {
					...
					while (i < expression.length() && Character.isDigit(current)) {
						i++;
					}
					
					String resultValue = expression.substring(startValue, endValue);
					nodeStack.push(new MyNode(resultValue));
					i--;
				}
				else if (current == '(') {
					...
					while (!operatorStack.isEmpty() && operatorStack.top() != '(') {
						MyNode<T> right = nodeStack.pop();
						MyNode<T> left = nodeStack.pop();
						char operator = operatorStack.pop();
						MyNode<T> parent = new MyNode(operator, left, right);
						nodeStack.push(parent);
					}
					operatorStack.pop(); // pop '('
				}
				else if (isOperator(current)) {
					while (!operatorStack.isEmpty() && operatorPriority(current) <= operatorPriority(operatorStack.top())) {
						MyNode<T> right = nodeStack.pop();
						MyNode<T> left = nodeStack.pop();
						char operator = operatorStack.pop();
						MyNode<T> parent = new MyNode(operator, left, right);
					...
					MyNode<T> operand = nodeStack.pop();
					char op = current;
					MyNode<T> parent = new MyNode(op, null, operand);
					nodeStack.push(parent);
				}
			}
